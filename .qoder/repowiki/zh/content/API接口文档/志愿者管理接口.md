...
## 项目结构
志愿者管理相关代码采用经典的三层架构（Controller-Service-Mapper），配合DTO/Entity模型与统一响应封装，形成清晰的职责边界与扩展点。现已新增值班申请相关组件。
...
- [AdminDutyApplicationController.java:18-131](file://src/main/java/com/daily/dailychineseculture/controller/AdminDutyApplicationController.java#L18-L131)
...
- [AdminDutyApplicationServiceImpl.java:18-72](file://src/main/java/com/daily/dailychineseculture/service/impl/AdminDutyApplicationServiceImpl.java#L18-L72)
...
- [AdminDutyApplicationController.java:25-131](file://src/main/java/com/daily/dailychineseculture/controller/AdminDutyApplicationController.java#L25-L131)
...
- [AdminDutyApplicationServiceImpl.java:29-72](file://src/main/java/com/daily/dailychineseculture/service/impl/AdminDutyApplicationServiceImpl.java#L29-L72)
- [ResponseResult.java](file://src/main/java/com/daily/dailychineseculture/common/ResponseResult.java)
- [JwtUtils.java](file://src/main/java/com/daily/dailychineseculture/util/JwtUtils.java)

## 架构总览
志愿者管理接口遵循REST风格，通过Controller接收请求，解析JWT获取用户ID，调用Service层业务逻辑，Service层通过Mapper访问数据库，最终以DTO封装返回。新增的值班申请流程同样遵循此架构模式。
...
participant S as "DutyApplicationService"
participant M as "DutyApplicationMapper"
participant R as "ResponseResult"
C->>DAC : POST /duty-application/submit
DAC->>JWT : 解析Authorization
JWT-->>DAC : userId
DAC->>S : submitApplication(userId, dto)
S->>M : 防重复申请检查
M-->>S : 检查结果
S->>M : 防重复授权检查
M-->>S : 检查结果
...
**新增** 管理员端接口提供完整的值班申请审批管理功能，包括统计分析和批量审批操作。

**章节来源**
- [AdminDutyApplicationController.java:25-131](file://src/main/java/com/daily/dailychineseculture/controller/AdminDutyApplicationController.java#L25-L131)
- [AdminDutyApplicationService.java:7-39](file://src/main/java/com/daily/dailychineseculture/service/AdminDutyApplicationService.java#L7-L39)
- [AdminDutyApplicationServiceImpl.java:18-72](file://src/main/java/com/daily/dailychineseculture/service/impl/AdminDutyApplicationServiceImpl.java#L18-L72)
- [AdminDutyApplicationMapper.java:15-76](file://src/main/java/com/daily/dailychineseculture/mapper/AdminDutyApplicationMapper.java#L15-L76)
...
- [AdminDutyApplicationServiceImpl.java:30-70](file://src/main/java/com/daily/dailychineseculture/service/impl/AdminDutyApplicationServiceImpl.java#L30-L70)

## 结论
本系统提供了完整的志愿者管理能力：从用户视角的历史与统计，到管理视角的成员与岗位管理，再到新增的值班申请管理，均以清晰的接口与稳定的实现支撑。通过JWT鉴权与数据库职责范围约束，实现了权限控制与数据安全。新增的值班申请功能包含完善的防重复校验和状态管理机制，管理员端提供基于角色的数据隔离和统计分析功能。建议在生产环境中完善日志体系、索引优化与缓存策略，持续提升性能与稳定性。
